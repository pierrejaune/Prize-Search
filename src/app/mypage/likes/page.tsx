...
export const metadata: Metadata = {
  title: 'お気に入り一覧ページ | マイページ | Prize Search',
...
export default async function LikesPage() {
  ...
  return (
    <div className='p-6'>
      <h2 className='text-xl lg:text-3xl font-bold my-4'>お気に入り一覧</h2>
      {products.length > 0 ? (
        <ul className='space-y-4'>
          {products.map((product) => (
            <li
              key={product.id}
              className='p-4 border rounded shadow relative flex hover:bg-gray-100'
            >
              <Link
